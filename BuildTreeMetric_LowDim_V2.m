function [TM, XX_VertexID] = BuildTreeMetric_LowDim_V2(XX, L)

% Adapted from BuildTreeMetric_HighDim_V2

% QuadTree % suppose dim=2
% Building upon BuildTreeMetric_HighDim_V2(XX, L, KC)
% Replace clustering by partition

% Update: July 11th
% FIXED 0-length for small set of supports (especially heavily duplicated
% supports)

% Update: July 4th
% REPLACE: if ~isempty(idZZ_idCCPP)
% BY: if length(idZZ_idCCPP) > 1
% WORK for small finite number of supports
% ---ALSO SKIP if there is only 1 data point
...
dim = size(XX{1}, 2); % SUPPOSE DIM=2 (LOW-DIM)
...
% (min; max) for each dim
cube_org = GetHyperCube(allXX); % 2 x dim
cube = RandomExpandHyperCube(cube_org); % RANDOMLY EXPANDING !!!
...
kcCenterPP = GetCenterHyperCube(cube); % column vector (dim x 1)
...
% % %             [K, clusterIndex, clusterCenter] = BinaryDim_Partition(d, N, x) 

           [rKCLL_idCCPP, idZZLL_idCCPP, kcCenterLL_idCCPP] = ...
                BinaryDim_Partition(dim, length(idZZ_idCCPP), allZZ_idCCPP');
...
% %                     idZZLL_idCCPP(idZZLL_idCCPP == clusterID_ZeroLength(iiCC_Zero)) = -1;
% %                 end
